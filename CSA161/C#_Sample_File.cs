 ...
 */
using System;                                       // Gives us access to a lot of system specific type groups. In particular, the Console class.
using System.Collections.Generic;                   // I'm actually not sure why this is here. This program doesn't use any collections
using System.Linq;                                  // Same thing here. The program compiles just fine without Collections or Linq
using System.Text;                                  // Allows for different text encodings (such as UTF-8)
namespace PrimeNumber                               // Program namespace
{
    class Program                                   // Main program class
    {
        ...
        {                                               // Main -> method name. string[] args -> takes an array of strings as a command line input.
                                                        // although this is superfluous since this program uses no command line arguments.
            ...
            for (int i = 2; i <= 100; i++)          // Loop through every number between 2 and 100 (we know 1 is not prime)
            )
            {
                for (int j = 2; j <= 100; j++)      // For each number in the i loop, loop through 2->100 again to do operations on the numbers
                {
                    ...
                    {                                   // multiples of each other. If the modulo of i and j ever equal 0, 
                        isPrime = false;                // excepting when i == j, the boolean is set to false,
                        break;                      // and the subloop is broken.
                    }
 
                }
                if (isPrime)                        // If after the j loop finishes or is broken, the boolean is still true, we can 
